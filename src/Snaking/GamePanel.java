...
public class GamePanel extends JPanel implements ActionListener{
	...
	public void checkCollisions() {
		...
		//Checa se a cabeça colide com alguma parte do corpo
		for(int i = bodyParts; i>0; i--) {
			if((x[0] == x[i]) && (y[0] == y[i])) {
				running = false;
			}
		...
		//Checa se a cabeca colide com a parede esquerda
		if(x[0]<0) {
			running = false;
		}
		
		//Checa se a cabeca colide com a parede direita
		if(x[0]>SCREEN_WIDTH) {
			running = false;
		}
		//Checa se a cabeca colide com a parede de cima
		if(y[0]<0) {
			running = false;
		}
		//Checa se a cabeca colide com a parede de baixo
